          ...
          <div className="w-full flex flex-col items-center gap-4">
            ...
            <div className={`absolute flex gap-2 h-full transition-all duration-300 ${isSearchVisible ? 'opacity-100 scale-100' : 'opacity-0 scale-95 pointer-events-none'}`}>
              <input
                ...
                className="px-4 py-3 rounded-lg border-1 border-gray-300 focus:outline-none focus:border-yellow-400 h-full"
              />
              <button
            ...
            <div className={`absolute flex gap-1 h-full transition-all duration-300 ${!isSearchVisible ? 'opacity-100 scale-100' : 'opacity-0 scale-95 pointer-events-none'}`}>
              ...
              <div className="relative h-full w-32">
                ...
                {isDropdownOpen && (
                  <div className="absolute right-0 mt-0 w-full bg-white border border-yellow-400 rounded-b-lg shadow-lg z-10 overflow-hidden border-t-0">
                    <button
                      onClick={() => { setIsProMode(false); setIsDropdownOpen(false); }}
                      className={`w-full text-left px-4 py-2 hover:bg-gray-100 text-gray-700 ${!isProMode ? 'bg-gray-50 font-semibold' : ''}`}
                    >
                      Standard
                    </button>
                    <button
